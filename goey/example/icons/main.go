// This package provides an application built using the goey package that
// demonstrates using a custom widget.  Although in the repository, the custom
// widget is a separate package, and does not have priviledged access to the
// internals.  The widget creates a small image to show an icon,, determined
// by the specified rune.
package main
...
func update() {
	// Update the controls in the window.
	...
	return &goey.Padding{
		...
		Child: &goey.VBox{
			...
			Children: []base.Widget{
				&goey.P{Text: "This application can be used to scroll the the icons provided by the Material Design Icons."},
				&goey.IntInput{
					Value: int64(iconIndex),
					Max:   int64(len(iconData) - 1),
					OnChange: func(v int64) {
						iconIndex = int(v)
						update()
					},
				},
				icons.Icon(iconData[iconIndex].Rune),
				&goey.Label{Text: iconData[iconIndex].Description},
			},
		},
	}
}
